...
export type ReturBarangRow = {
  code: string;
  name: string;
  finalDo: number;
  qtyRetur: number; // abs(item_qty_revision) when revision is negative
  caseQty?: number | null;
  balQty?: number | null;
  slopQty?: number | null;
  packQty?: number | null;
  ...
  const totals = useMemo(() => {
    return rows.reduce(
      (acc, row) => ({
        ...
        qtyRetur: acc.qtyRetur + (Number(row.qtyRetur) || 0),
        ...
        <div className="flex items-center justify-between px-6 py-4 bg-white border-b print:hidden">
          <div className="flex items-center space-x-2">
            ...
            <h2 className="text-lg font-semibold text-gray-800">
              Preview Form Retur ke Gudang Utama
            </h2>
          </div>
            ...
            <table className="w-full text-sm border-collapse">
              ...
              <tbody>
                {rows.length === 0 ? (
                  ...
                  rows.map((row, idx) => (
                    <tr key={`${row.code}-${idx}`}>
                      ...
                      <td className="border-2 border-dashed border-gray-800 p-2 text-center text-red-600 font-bold">
                        {formatQty(row.qtyRetur)}
                      </td>
                      <td className="border-2 border-dashed border-gray-800 p-2 text-center text-red-500">
                ...
                <tr>
                  ...
                  <td className="border-2 border-dashed border-gray-800 p-2 text-center font-bold text-red-600">
                    {formatQty(totals.qtyRetur)}
                  </td>
                  <td className="border-2 border-dashed border-gray-800 p-2 text-center font-bold">
