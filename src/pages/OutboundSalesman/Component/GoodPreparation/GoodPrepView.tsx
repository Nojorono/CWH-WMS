...
function GoodPrepView({
  ...
  return (
    <div className="min-h-screen space-y-4 bg-gray-50 p-6 font-sans">
          ...
          headerActions={
            ...
            <div className="ml-auto flex items-center gap-2">
              ...
              <button
                ...
                disabled={isPrintDisabled || returReportRows.length === 0}
                className={`flex items-center gap-2 rounded-lg px-4 py-2 text-xs font-semibold shadow-sm transition-colors ${
                  isPrintDisabled || returReportRows.length === 0
                    ? "cursor-not-allowed bg-slate-200 text-slate-400"
              ...
              <button
                type="button"
                onClick={() => setIsTambahanOpen(true)}
                disabled={isPrintDisabled}
                className={`flex items-center gap-2 rounded-lg px-4 py-2 text-xs font-semibold shadow-sm transition-colors ${
                  isPrintDisabled
                    ? "cursor-not-allowed bg-slate-200 text-slate-400"
      ...
      {isTambahanOpen && (
        <TambahanBarang
          onClose={() => setIsTambahanOpen(false)}
          organizationName={String(organization_name || "-")}
