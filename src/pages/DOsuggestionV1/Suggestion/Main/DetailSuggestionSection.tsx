...
export default function DetailSuggestionSection() {
  ...
  // ============================================================================
  const totalSku = useMemo(() => localDetails.length, [localDetails]);

  const totalQty = useMemo(() => {
    return localDetails.reduce((sum, item) => {
      const sku = item.item_code;
      // Jika sedang ada di state revisi (input inline), pakai nilai revisi terbaru
      if (revisions.has(sku)) {
        return sum + (revisions.get(sku) || 0);
      }
      // Jika tidak, gunakan nilai final/revision/suggestion dari API
      const qty =
        Number(item.item_qty_final) ||
        Number(item.item_qty_revision) ||
        Number(item.item_qty_suggestion) ||
        0;
      return sum + qty;
    }, 0);
  }, [localDetails, revisions]);
