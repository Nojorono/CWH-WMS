...
const AdjustTable = ({
  ...
  //       accessorKey: "CHANNEL",
  //       header: () => (
  //         <div className="text-left whitespace-nowrap">CHANNEL</div>
  //       ),
  //       cell: ({ row }) => (
  ...
  //       header: () => (
  //         <div className="text-left whitespace-nowrap">CALLPLAN START DATE</div>
  //       ),
  //       cell: ({ row }) => {
  ...
  //       header: () => (
  //         <div className="text-left whitespace-nowrap">CALLPLAN END DATE</div>
  //       ),
  //       cell: ({ row }) => (
  //         <span className="text-sm text-slate-600 whitespace-nowrap">
  ...
  //     },
  //     {
  //       accessorKey: "CALL_PLAN_START_PERIOD",
  //       header: () => (
  //         <div className="text-left whitespace-nowrap">START PERIOD</div>
  //       ),
  //       cell: ({ row }) => (
  //         <span className="text-sm text-slate-600 whitespace-nowrap">
  //           {row.original.CALL_PLAN_START_PERIOD || "-"}
  //         </span>
  //       ),
  //     },
  //     {
  //       accessorKey: "CALL_PLAN_END_PERIOD",
  //       header: () => (
  //         <div className="text-left whitespace-nowrap">END PERIOD</div>
  //       ),
  //       cell: ({ row }) => (
  //         <span className="text-sm text-slate-600 whitespace-nowrap">
  //           {row.original.CALL_PLAN_END_PERIOD || "-"}
  //         </span>
  //       ),
  //     },
  //     {
  ...
  //         const startPeriod = row.original.CALL_PLAN_START_PERIOD;

  //         if (!row.original.CALL_PLAN_NUMBER?.trim()) {
  //           return null;
  //         }

  //         // Rule: CALL_PLAN_START_PERIOD !== CALL_PLAN_START_DATE -> tidak perlu di generate
  //         const isPeriodMismatch = startPeriod && startDate && startPeriod !== startDate;
  //         const isAllowedToGenerate = !isPeriodMismatch && isGenerateDOAllowed(startDate);

  //         // Susun pesan error berdasarkan alasan tombol terkunci
  //         let errorMessage = "";
  //         if (isPeriodMismatch) {
  //           errorMessage = "Periode awal berbeda dengan Tanggal Mulai Call Plan (Tidak perlu di-generate)";
  //         } else if (startDate) {
  //           errorMessage = getGenerateErrorMessage(startDate);
  //         } else {
  ...
  const columns: ColumnDef<CallPlanDetail>[] = useMemo(
    () => [
      ...
      // Kolom 1: Tanggal Kunjungan Bertumpuk (Start & End Date)
      {
        id: "callplan_dates",
        header: () => (
          <div className="text-left whitespace-nowrap">TANGGAL KUNJUNGAN</div>
        ),
        cell: ({ row }) => {
          const callPlan = row.original.CALL_PLAN_NUMBER;
          const hasCallPlan = callPlan !== null && callPlan !== "";
          const startDate = row.original.CALL_PLAN_START_DATE;
          const endDate = row.original.CALL_PLAN_END_DATE;

          return (
            <div className="flex flex-col text-sm text-slate-700 whitespace-nowrap">
              <span className="font-semibold text-slate-700">{hasCallPlan ? startDate : "-"}</span>
              {hasCallPlan && endDate && (
                <span className="text-xs text-slate-400">s/d {endDate}</span>
              )}
            </div>
          );
        },
      ...
      // Kolom 2: Periode Siklus Bertumpuk (Start & End Period)
      {
        id: "callplan_periods",
        header: () => (
          <div className="text-left whitespace-nowrap">PERIODE SIKLUS</div>
        ),
        cell: ({ row }) => {
          const startPeriod = row.original.CALL_PLAN_START_PERIOD;
          const endPeriod = row.original.CALL_PLAN_END_PERIOD;

          return (
            <div className="flex flex-col text-sm text-slate-700 whitespace-nowrap">
              <span className="font-medium text-slate-600">{startPeriod || "-"}</span>
              {endPeriod && (
                <span className="text-xs text-slate-400">s/d {endPeriod}</span>
              )}
            </div>
          );
        },
  ...
  const handleGenerateDO = async (rowData: any) => {
    ...
    try {
      const startPeriod = rowData.CALL_PLAN_START_PERIOD;
      const startDate = rowData.CALL_PLAN_START_DATE;

      // Proteksi tambahan sebelum hit API
      const isPeriodMismatch = startPeriod && startDate && startPeriod !== startDate;

      if (isPeriodMismatch) {
        showErrorToast("Periode awal berbeda dengan Tanggal Mulai Call Plan (Tidak perlu di-generate)");
        return;
      }

      if (!isGenerateDOAllowed(startDate)) {
        showErrorToast(getGenerateErrorMessage(startDate));
        return;
      }
