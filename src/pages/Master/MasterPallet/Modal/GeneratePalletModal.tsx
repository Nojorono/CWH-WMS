...
const GeneratePalletModal: React.FC<GeneratePalletModalProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // 🔑 Ambil organization_id otomatis dari index 0
    const autoOrgId =
      organizations.length > 0 ? organizations[0].organization_id : null;

    if (!autoOrgId) {
      showErrorToast("Organization ID tidak ditemukan.");
      setLoading(false);
      return;
    }
    ...
    const payload = {
      ...formData,
      organization_id: Number(autoOrgId), // Masuk ke payload otomatis
      start: Number(formData.start),
      end: Number(formData.end),
      padding: Number(formData.padding),
      capacity: Number(formData.capacity),
    ...
    console.log("Payload generate dikirim:", payload);
      ...
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
        ...
        <div className="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
          <div className="flex flex-col">
            ...
            {/* Info Organisasi Kecil di bawah title */}
            {organizations.length > 0 && (
              <span className="text-[10px] text-gray-500 font-medium">
                Target: {organizations[0].organization_name} (ID:{" "}
