...
const WarehouseMapView = ({ data }: { data: any[] }) => {
  ...
  return (
<div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden transition-all duration-300 p-6">      {/* --- HEADER CONTROL --- */}
      <div 
      ...
      >
        <div className="flex items-center gap-4">
          <div className={`p-3 rounded-xl transition-colors ${isExpanded ? "bg-blue-50 text-blue-600" : "bg-blue-500 text-white"}`}>
            <FaWarehouse size={24} />
          </div>
          <div>
            <h2 className={`text-xl font-bold transition-colors ${isExpanded ? "text-gray-800" : "text-white"}`}>
              Warehouse Visual Map
            </h2>
            <p className={`text-sm ${isExpanded ? "text-gray-500" : "text-blue-100"}`}>
              Monitoring {Object.keys(zones).length} Active Zones
            </p>
          </div>
        </div>
        
        <div className={`flex items-center gap-3 transition-all duration-500 ${isExpanded ? "rotate-180" : "rotate-0"}`}>
          <div className={`p-2 rounded-full ${isExpanded ? "bg-gray-100 text-gray-400" : "bg-white/20 text-white"}`}>
      ...
      {/* --- MAIN CONTENT AREA --- */}
      <div className={`grid grid-cols-1 lg:grid-cols-2 gap-8 transition-all duration-500 ease-in-out origin-top
        ${isExpanded ? "opacity-100 scale-100 h-auto" : "opacity-0 scale-95 h-0 overflow-hidden"}`}>
        
        {Object.keys(zones).map((zoneName) => (
          <div key={zoneName} className="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden flex flex-col">
            {/* Zone Header */}
            <div className="bg-white px-6 py-4 border-b border-gray-50 flex justify-between items-center">
              <div className="flex items-center gap-2">
                <div className="w-2 h-6 bg-blue-600 rounded-full" />
                <h3 className="text-gray-800 font-extrabold tracking-tight">ZONE {zoneName}</h3>
              </div>
              ...
              {Object.keys(zones[zoneName]).map((binName) => (
                <div key={binName} className="flex flex-col gap-2 group">
                  <div className="flex items-center justify-between px-1">
                    <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Bin {binName}</span>
                  </div>
                  ...
                  <div className="min-h-[120px] p-3 bg-white border-2 border-dashed border-gray-200 rounded-[2rem] flex flex-wrap gap-2 transition-colors group-hover:border-blue-200 shadow-inner">
                    {zones[zoneName][binName].map((pallet: any) => (
                      ...
                      >
                        <FaBox className={pallet.bad_inventory.length > 0 ? "text-red-500" : "text-blue-500"} size={22} />
                        <span className="text-[10px] font-black mt-2 text-gray-700 truncate w-full px-2">{pallet.pallet_code}</span>
      ...
      {!isExpanded && (
        // <div className="flex flex-col items-center justify-center py-20 bg-white rounded-3xl border-2 border-dashed border-gray-200 animate-in fade-in zoom-in duration-300">
        //    <FaWarehouse className="text-gray-200 mb-4" size={60} />
        //    <p className="text-gray-400 font-medium italic">Layout map is hidden. Click the header to expand.</p>
        // </div>
        <></>
      )}
      ...
      {/* --- SIDEBAR DETAIL --- */}
      {selectedPallet && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[100] flex justify-end transition-all duration-500">
          <div className="w-full max-w-md bg-white h-full shadow-2xl p-8 overflow-y-auto animate-in slide-in-from-right duration-300 border-l border-white/20">
            <div className="flex justify-between items-center mb-10">
              <div>
                <h3 className="text-2xl font-black text-gray-900 tracking-tight">Pallet Intelligence</h3>
                <p className="text-sm text-gray-500">Detailed stock breakdown</p>
              </div>
              <button 
            ...
            <div className="space-y-6">
              ...
              <div className="relative p-6 bg-gradient-to-br from-blue-600 to-blue-700 rounded-3xl shadow-xl shadow-blue-100 overflow-hidden">
                ...
                <p className="text-blue-200 text-[10px] uppercase font-black tracking-widest mb-1">Pallet Identity</p>
                <h4 className="text-3xl font-black text-white">{selectedPallet.pallet_code}</h4>
              ...
              <div className="space-y-4 pt-4">
                <h5 className="text-xs font-black text-gray-400 uppercase tracking-widest px-1">Inventory Items</h5>
                
                {/* Jika ada current_items tampilkan, jika tidak dan ada bad_inventory tampilkan bad_inventory */}
                {selectedPallet.current_items && selectedPallet.current_items.length > 0 ? (
                  // Normal Items
                  selectedPallet.current_items.map((item: any, i: number) => (
                    <div key={i} className="flex justify-between items-center p-4 bg-gray-50 rounded-2xl border border-transparent hover:border-blue-100 hover:bg-white transition-all">
                      <div>
                        <p className="text-sm font-bold text-gray-800">{item.item_name}</p>
                        <p className="text-[10px] text-gray-400 font-medium">
                          Prod. Date: {item.production_date ? new Date(item.production_date).toLocaleDateString() : '-'}
                        </p>
                      </div>
                      <div className="text-right">
                        <p className="text-sm font-black text-blue-600">{item.current_quantity}</p>
                        <p className="text-[9px] text-gray-400 font-bold">{item.uom}</p>
                      </div>
                    </div>
                  ))
                ) : selectedPallet.bad_inventory && selectedPallet.bad_inventory.length > 0 ? (
                  // Jika tidak ada current_items, tampilkan bad_inventory
                  selectedPallet.bad_inventory.map((bad: any, i: number) => (
                    <div key={i} className="flex flex-col p-4 bg-red-50 rounded-2xl border border-red-100 relative overflow-hidden">
                      ...
                      <div className="flex justify-between items-start mb-2">
                        <div>
                          <p className="text-sm font-bold text-red-900">{bad.item_name || bad.item_id}</p>
                          <p className="text-[10px] text-red-400 font-medium">
                            Prod. Date: {bad.production_date ? new Date(bad.production_date).toLocaleDateString() : '-'}
                          </p>
                          <p className="text-[10px] text-red-400 font-medium">
                            Year: {bad.year || "-"}
                          </p>
                        ...
                        <div className="text-right">
                          <p className="text-sm font-black text-red-600">{bad.quantity}</p>
                          <p className="text-[9px] text-red-400 font-bold uppercase">{bad.uom}</p>
                        </div>
                      </div>
                  ...
                  <div className="text-center text-gray-400 italic text-xs py-6">Tidak ada item</div>
                )}
              </div>
            </div>
          </div>
