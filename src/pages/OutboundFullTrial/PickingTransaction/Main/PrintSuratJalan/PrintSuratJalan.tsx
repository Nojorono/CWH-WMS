...
//   const { fetchById, detail, updateData } = useStoreOutboundDeliveryOrder();
...
//   const handleUpdate = (sealNumber: string) => {
  ...
  return (
    <div className="p-6 bg-slate-50 min-h-screen font-sans select-none">
      <div className="max-w-6xl mx-auto">
        <header className="flex justify-between items-center mb-8">
          <div>
            <h1 className="text-2xl font-bold text-slate-800">Print Surat Jalan</h1>
            <p className="text-slate-500 text-sm">Pratinjau dokumen operasional sebelum dicetak</p>
          </div>

          <Button variant="primary" onClick={() => navigate(-1)} startIcon={<FaArrowLeft />}>
            Back to List DO
          </Button>
        </header>
          ...
          <table className="w-full text-left border-collapse">
            <thead className="bg-slate-50 border-b border-slate-200">
              <tr>
                ...
                <th className="p-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">No. Memo</th>
                <th className="p-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Customer</th>
                <th className="p-4 text-xs font-semibold text-slate-500 uppercase tracking-wider w-1/3">Alamat</th>
                <th className="p-4 text-xs font-semibold text-slate-500 uppercase tracking-wider text-center">Aksi</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100">
              {detail?.outbound_memos?.map((memo: any, idx: number) => (
                <tr key={memo.id} className="hover:bg-blue-50/50 transition tracking-tight">
                  <td className="p-4 text-sm text-slate-600 font-medium">{idx + 1}</td>
                  <td className="p-4 text-sm text-blue-600 font-bold">{memo.outbound_memo_number}</td>
                  <td className="p-4 text-sm text-slate-700 font-semibold">{memo.requestor}</td>
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-5xl h-[90vh] flex flex-col overflow-hidden animate-in fade-in zoom-in duration-200">
            ...
            <div className="p-4 border-b border-slate-100 flex justify-between items-center bg-slate-50">
              <div className="flex flex-col">
                <h3 className="font-bold text-slate-800 text-lg">Preview Surat Jalan</h3>
                <span className="text-xs text-slate-500 uppercase tracking-widest font-semibold">
                  Memo: {selectedMemo.outbound_memo_number}
                </span>
              </div>
              <button onClick={closeModal} className="text-slate-400 hover:text-slate-600 text-3xl leading-none">&times;</button>
            </div>
