...
interface TableComponentProps<T> {
  ...
  totalPages?: number;
  isLoading?: boolean;
  // Tambahkan prop ini untuk merender detail row
  renderSubComponent?: (props: { row: any }) => React.ReactElement;
}

const MainTable = <T extends { [key: string]: any }>({
  data,
  columns,
  globalFilter,
  setGlobalFilter,
  ...
  const table = useReactTable<T>({
    ...
    getExpandedRowModel: getExpandedRowModel(), // Aktifkan model expand
    enableRowSelection: !!selectionColumn,
    getRowCanExpand: () => true, // Izinkan semua baris bisa di-expand
  });
  ...
  return (
    <div className="flex flex-col bg-white rounded-xl shadow-md border border-gray-200 overflow-hidden w-full">
              ...
              {table.getRowModel().rows.length === 0 && !isLoading ? (
                ...
                table.getRowModel().rows.map((row) => (
                  <Fragment key={row.id}>
                    ...
                    {/* BARIS EXPAND (Detail/SubComponent) */}
                    {row.getIsExpanded() && renderSubComponent && (
                      <tr>
                        <td
                          colSpan={row.getVisibleCells().length}
                          className="p-0 border-b border-gray-200 bg-slate-50/50"
                        >
                          {/* Render MemoCell disini */}
                          <div className="w-full shadow-inner animate-in fade-in slide-in-from-top-2 duration-300">
      ...
      <div className="px-6 py-3 bg-gray-50 border-t border-gray-200 flex flex-col sm:flex-row items-center justify-between gap-4">
        {/* ... Kode Pagination Anda (Tidak diubah) ... */}
