  ...
  const columns: ColumnDef<OutboundDoUI>[] = useMemo(
    () => [
      ...
      },
      {
        ...
        id: "transaction_type",
        cell: ({ row }) => {
          const itemLog = row.original.outbound_memos?.[0]
            ?.outbound_memo_items?.[0] as any;
          const txType = itemLog?.current_transaction_type || "-";
          const sourceSystem =
      ...
      },
      {
        ...
        cell: ({ row }) => {
          const itemLog = row.original.outbound_memos?.[0]
            ?.outbound_memo_items?.[0] as any;
          const intg = itemLog?.integration_data;
          const txType = itemLog?.current_transaction_type;
