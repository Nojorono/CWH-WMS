...
import {
    ...
    CreateUserPayload,
    UpdateUserPayload,
} from "../../services/MasterServices/MasterUserService";
...
export const useUserStore = create<UserStore>((set) => ({
    ...
    createUser: async (payload: CreateUserPayload) => {
        ...
        try {
            ...
            await createUserService(filteredPayload);
            const users = await fetchAllUser();
            set({ user: users, loading: false });
            return { ok: true };
        } catch (err: any) {
    ...
    fetchDetailUser: async (id: string) => {
        set({ loading: true, error: null });
        try {
            const userDetail = await fetchDetailUser(id);
            set({ userDetail });
    ...
    updateUser: async (id: string, payload: UpdateUserPayload) => {
        ...
        try {
            ...
            await updateUser(id, filteredPayload);
            const users = await fetchAllUser();
            set({ user: users, loading: false });
            return { ok: true };
        } catch (err: any) {
