        ...
        >
          {show ? (
            <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M13.875 18.825A10.05 10.05 0 0112 19c-5 0-9-4-9-7 0-1.14.504-2.17 1.3-3.07M6.1 6.1A9.956 9.956 0 0112 5c5 0 9 4 9 7a9.97 9.97 0 01-2.1 3.9M15 12a3 3 0 11-6 0 3 3 0 016 0zM3 3l18 18" />
            </svg>
          ) : (
            <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
...
const UsernameField: React.FC<{
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (val.includes(" ")) {
      ...
      setHint("💡 Disarankan menggunakan snake_case (kecil, angka, underscore).");
    } else {
      setHint(null);
    }
  };
  ...
  return (
    <div>
      ...
      />
      {hint && (
        <p className={`text-xs mt-1 ${hint.startsWith("⚠️") ? "text-red-500" : "text-yellow-600"}`}>
          {hint}
        </p>
      )}
    </div>
...
const ModalForm: React.FC<ModalFormProps> = ({
  ...
  const inputCls = "w-full px-3 py-2 border rounded-md focus:outline-none focus:ring focus:ring-blue-300 transition-all";
  const disabledCls = "w-full px-3 py-2 border rounded-md bg-gray-100 cursor-not-allowed text-gray-500";
      ...
      case "select":
        return (
          <Controller
            ...
            control={control}
            rules={{
              ...(field.validation?.required ? { validate: (v) => !!v || field.validation?.required || "Required" } : {}),
              ...field.validation,
            }}
            render={({ field: controllerField }) => (
              <Select
                ...
                placeholder="Select..."
                classNamePrefix="react-select"
                value={field.options?.find((opt) => opt.value === controllerField.value)}
        ...
        return (
          <UsernameField
            registerProps={register(field.name, {
              ...field.validation,
              validate: {
                noSpaces: (v: string) => !v?.includes(" ") || "Tidak boleh ada spasi",
              },
            })}
            isDisabled={isDisabled}
            inputCls={inputCls}
        ...
        return (
          <Controller
            ...
            render={({ field: ctrl }) => (
              <DatePicker
                id={field.name}
                onChange={(date: any) => ctrl.onChange(Array.isArray(date) ? date[0] : date)}
                readOnly={isDisabled}
              />
            )}
          />
  ...
  return (
    <div className="mx-auto p-4 rounded-md bg-white">
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        <div className={`grid ${right.length > 0 ? "grid-cols-1 md:grid-cols-2" : "grid-cols-1"} gap-x-8 gap-y-4`}>
            ...
            <div key={gIdx} className="space-y-4">
              {group.map((field) => (
                <div key={field.name}>
                  ...
                  {renderField(field)}
                  {errors[field.name] && (
                    <p className="text-red-500 text-xs mt-1">{(errors[field.name] as any).message}</p>
                  )}
                </div>
              ))}
            </div>
